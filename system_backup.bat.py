...
def car_delivery():
    ...
    try:
        conn = get_db_connection()
        deliveries = conn.execute('SELECT * FROM car_delivery ORDER BY id DESC').fetchall()
        conn.close()
        ...
        for delivery in deliveries:
            ...
            <tr>
                ...
                <td>{delivery['notes'] or 'لا توجد'}</td>
            </tr>
            """
        ...
        <body>
            ...
            <div class="container mt-4">
                ...
                <div class="card">
                    <div class="card-header">
                        <h5>سجل التسليمات ({{ deliveries|length }} تسليم)</h5>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped">
        ...
        ''', deliveries=deliveries, deliveries_html=deliveries_html)
        
    except Exception as e:
        return f"خطأ في عرض بيانات التسليم: {str(e)}"
...
def add_delivery():
    ...
    if request.method == 'POST':
        try:
            ...
            if not all([delivery_date, car_info, recipient_name]):
                flash('يرجى ملء جميع الحقول المطلوبة!', 'error')
                return redirect(url_for('add_delivery'))
            
            conn = get_db_connection()
            conn.execute('''
                INSERT INTO car_delivery (delivery_date, car_info, recipient_name, recipient_phone, delivery_location, status, notes, created_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (delivery_date, car_info, recipient_name, recipient_phone, delivery_location, 'مسلم', notes, session.get('username', 'unknown')))
            
            conn.commit()
            conn.close()
            
            flash(f'تم إضافة تسليم السيارة {car_info} للمستلم {recipient_name} بنجاح!', 'success')
            return redirect(url_for('car_delivery'))
            
        except Exception as e:
            flash(f'خطأ في إضافة التسليم: {str(e)}', 'error')
            return redirect(url_for('add_delivery'))
